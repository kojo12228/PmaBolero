...
type ProjectService(ctx: IRemoteContext, env: IWebHostEnvironment) =
    ...
    override this.Handler =
        { createProject =
            ctx.AuthorizeWith [ AuthorizeAttribute(Roles = "admin, pm") ]
            <| fun newProj ->
                async {
                    let newProjId = Backend.getNextProjectId ()

                    let validDeptId () =
                        Map.containsKey newProj.DepartmentId Backend.departments
                    ...
                    if (validDeptId ()
                       ...
                       |> not then
                        ...
                        Backend.departmentProjects <-
                            Map.add newProj.DepartmentId newDeptProjs Backend.departmentProjects
                  ...
                  async {
                      ...
                      let deptOpt =
                          ...
                          |> Option.bind (fun _ ->
                              Backend.departmentProjects
                              |> Map.toSeq
                              |> Seq.tryFind (fun (_, projIds) -> Set.contains projId projIds))

                      let newDeptProjsOpt = Map.tryFind newDeptId Backend.departmentProjects
          ...
          updateProject =
              ...
              <| fun (projDetails) ->
                  async {
                      ...
                      let newProjDevsSet =
                          if allNewDevIdsAreDevs then
                              Some <| Set.ofArray projDetails.DeveloperIds
                          else
                              None
                  ...
                  async {
                      ...
                      let pmOptOpt =
                          ...
                          | Some pmId ->
                              ...
                              match pmOpt with
                              | Some pm ->
                                  if pm.Role = ProjectManager then
                                      Some <| Some pmId
                                  else
                                      None
                              | None -> None
              ...
              <| fun projId ->
                  async {
                      ...
                      | Some _ ->
                          ...
                          Backend.departmentProjects <-
                              Map.map (fun _ projIds -> Set.remove projId projIds) Backend.departmentProjects
